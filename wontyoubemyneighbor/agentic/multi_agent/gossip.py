"""
Gossip Protocol for RubberBand-to-RubberBand Communication

Implements epidemic-style information dissemination between RubberBand instances
...
@dataclass
class GossipMessage:
    """Message exchanged between RubberBand instances"""
    message_id: str
...
class GossipProtocol:
    """
    Gossip protocol for distributed RubberBand coordination.
    ...
    def register_peer(
        ...
        """Register a RubberBand peer for gossip"""
        self.peers[peer_id] = {
    ...
    def create_message(
        ...
        message_data = f"{self.rubberband_id}{datetime.utcnow().isoformat()}{json.dumps(payload)}"
        message_id = hashlib.sha256(message_data.encode()).hexdigest()[:16]

        return GossipMessage(
            message_id=message_id,
            message_type=message_type,
            sender_id=self.rubberband_id,
    ...
    async def broadcast(self, message: GossipMessage):
        ...
        message.seen_by.add(self.rubberband_id)
    ...
    async def start(self):
        ...
        print(f"[Gossip] Started for RubberBand {self.rubberband_id}")
